...
export default {
  ...
  async beforeMount() {
    let result1 = await this.axios.get(
      'https://wyymusicapi.vercel.app/top/playlist?cat=伤感&order=hot'
    )
...
.shanggan > ul li > .shangganN > .number1 {
  width: 180px;
  height: 40px;
  display: flex;
...
.shanggan > ul li > img:first-child {
  cursor: pointer;
  float: left;
}
.shanggan > ul li > img:first-child + img {
  margin-left: -55px;
  margin-top: 17px;
  cursor: pointer;
  float: left;
}
.shanggan > ul li > .shangganN > .number1 > span:first-child {
  font-size: 13px;
  overflow: hidden;
...
.shanggan > ul li > .shangganN > .number1 > span:first-child + span {
  font-size: 11px;
  color: #999;
  margin-top: 10px;
  overflow: hidden;
...
.shanggan > ul li > .shangganN > .number2 {
  width: 60px;
  height: 18px;
}
.shanggan > ul li > .shangganN > .number2 > span {
  color: #8d8d8d;
  font-size: 11px;
}
</style>
