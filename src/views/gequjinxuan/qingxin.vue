...
export default {
  ...
  async beforeMount() {
    let result1 = await this.axios.get(
      'https://wyymusicapi.vercel.app/top/playlist?cat=清新&order=hot'
    )
...
.qingxin > ul li > .qingxinN > .number1 {
  width: 180px;
  height: 40px;
  display: flex;
...
.qingxin > ul li > img:first-child {
  cursor: pointer;
  float: left;
}
.qingxin > ul li > img:first-child + img {
  margin-left: -55px;
  margin-top: 17px;
  cursor: pointer;
  float: left;
}
.qingxin > ul li > .qingxinN > .number1 > span:first-child {
  font-size: 13px;
  overflow: hidden;
...
.qingxin > ul li > .qingxinN > .number1 > span:first-child + span {
  font-size: 11px;
  color: #999;
  margin-top: 10px;
  overflow: hidden;
...
.qingxin > ul li > .qingxinN > .number2 {
  width: 60px;
  height: 18px;
}
.qingxin > ul li > .qingxinN > .number2 > span {
  color: #8d8d8d;
  font-size: 11px;
}
</style>
